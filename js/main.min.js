...
"use strict";
/* WEBPACK VAR INJECTION */(function(global) {/* harmony import */ var _helpers_bind_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4);
...
/* harmony import */ var _platform_node_classes_FormData_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(5);
...
var siblings = function siblings(elem) {
  var createSiblings = function createSiblings(n, elem) {
    var matched = [];
...
function singleDropdown(opts) {
  ...
  docClick = function docClick(e) {
    ...
    if (targetEl.classList.contains('menu__arrow') || targetEl.closest('.menu__arrow') != null) {
      // убежаемся, что кликнкнули по стрелке рядом со ссылкой
    ...
    if (!targetEl.closest('.menu__item') && document.querySelector('.menu__item.touch-hover') !== null) {
      //провереряем клик вне выпадающего меню
      removeClass('.menu__item', 'touch-hover'); //удаляем у всех пунктов меню активный класс
    }
  };
...
function ScrollToSects(opts) {
  var _self = this,
    ...
    links = Array.prototype.slice.call(document.querySelector(opts.linksContainer).querySelectorAll('[data-anchor]')),
...
/* harmony import */ var _libs_vanilla_service_functions_all_functions_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(6);
/* harmony import */ var _libs_vanilla_service_functions_all_functions_js__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_libs_vanilla_service_functions_all_functions_js__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _libs_vanilla_tiny_slider_dist_tiny_slider_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(8);
/* harmony import */ var _libs_vanilla_tiny_slider_dist_tiny_slider_js__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_libs_vanilla_tiny_slider_dist_tiny_slider_js__WEBPACK_IMPORTED_MODULE_1__);
...
//- remove-class--------------------------
var axios = __webpack_require__(17)["default"];
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  new _libs_vanilla_service_functions_all_functions_js__WEBPACK_IMPORTED_MODULE_0__["ScrollToSects"]({
    // linksContainer: 'header',//контейнер, в котором лежат кнопки навигации
    offset: 80,
    //отступ от верха экрана при прокрутке (если нужен)
    ...
    anchorSpy: true,
    //добавление активного класса ссылке при скролле, если соответствующая ей секция попадает в экран
    activeClassAdding: false //добавление классов активным ссылкам
  });

  if (document.querySelector('.reviews__sldr') !== null) {
    var slider = Object(_libs_vanilla_tiny_slider_dist_tiny_slider_js__WEBPACK_IMPORTED_MODULE_1__["tns"])({
      container: '.reviews__sldr',
      mode: 'carousel',
      //'gallery' - для фэйд-анимации отдельных слайдов
      items: 1,
      loop: false,
      slideBy: 1,
      // кол-во слайдов, перематывающихся за 1 клик. Не работает с mode: 'gallery'
      // autoplay: true,
      // controls: false, // отключение кнопок "вперед/назад"
      controlsContainer: '.tiny-carousel__nav',
      // внутри .block-header__nav должны быть 2 заранее отстилизованные кнопки
      ...
      mouseDrag: true,
      gutter: 10,
      //добавляет padding, а не margin! Нужна обертка вокруг содержимого каждого слайда!
      responsive: {
        // mobile first!
        375: {
  ...
  var cardsRender = function (data) {
    var cardsContainer = document.querySelector('.blog__cards');
    console.log('ubut!');
    var template = "";
    ...
    var pizrender = function pizrender(cards) {
      console.log(cards);
      cards.forEach(function (card) {
        template += '<article class="blog__card card-blog">';
        if (card.img) {
          template += "<a href=\"#\" class=\"card-blog__img _fit\"><img src=\"".concat(card.img, "\" alt=\"alt\" class=\"_fit-img\"></a>");
        }
        if (card.date) {
          template += "<div class=\"card-blog__date\">".concat(card.date, "</div>");
        }
        if (card.title) {
          template += "<h4 class=\"card-blog__title\"> <a href=\"#\">".concat(card.title, "</a></h4>");
        }
        if (card.text) {
          template += "<div class=\"card-blog__text\">".concat(card.text, "</div>");
        }
        if (card.tags && card.tags.length > 0) {
          template += '<div class="card-blog__tags">';
          card.tags.forEach(function (tag) {
            return template += "<a href=\"#\" class=\"tag\">".concat(tag, "</a>");
          });
          template += '</div>';
        }
        template += '</article>';
    ...
    var cardsFetch = function cardsFetch(length) {
      console.log('length in fetch=' + length);
      axios.get('../src/data/data.json').then(function (response) {
        // handle success
...
(function () {
  var detailsSpoilers = function detailsSpoilers(siblings) {
    var spoilers = document.querySelectorAll('[data-spoiler-item]'),
      minimizeSiblings = siblings;
    init = function init() {
      spoilersCycle();
    };
    spoilersCycle = function spoilersCycle() {
      Array.prototype.slice.call(spoilers).forEach(function (item) {
        var question = item.querySelector('[data-spoiler-title]'),
          ...
          setHeight = function setHeight() {
            answer.style.setProperty('--details-height', answer.scrollHeight + 'px');
          };
    ...
    clickListener = function clickListener(e) {
      ...
      var siblings = Array.prototype.slice.call(item.parentNode.children).filter(function (el) {
        return el.classList.contains('js-details');
      }).filter(function (el) {
        return el !== event.target.parentNode;
      });
...
__webpack_require__.d(__webpack_exports__, "getAdapter", function() { return /* binding */ getAdapter; });
...
var toFormData = __webpack_require__(2);
...
var helpers_null = __webpack_require__(5);
...
/* harmony default export */ var xhr = (isXHRAdapterSupported && function (config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    if (utils["a" /* default */].isFormData(requestData)) {
      ...
      } else if(utils["a" /* default */].isString(contentType = requestHeaders.getContentType())){
    ...
    if (browser.isStandardBrowserEnv) {
      ...
      const xsrfValue = helpers_isURLSameOrigin(fullPath) && config.xsrfCookieName && cookies.read(config.xsrfCookieName);
